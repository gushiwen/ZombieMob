...
from Course_MyLibrary import *

def calc_velocity(direction, vel=1.0):
    velocity = Point(0,0)
    # 0 UP, 2 RIGHT, 4 DOWN, 6 LEFT
    if direction == 0: velocity.y = -vel
    elif direction == 2: velocity.x = vel
    elif direction == 4: velocity.y = vel
    elif direction == 6: velocity.x = -vel
    return velocity

def reverse_direction(sprite):
    if sprite.direction == 0: sprite.direction = 4
    elif sprite.direction == 2: sprite.direction = 6
    elif sprite.direction == 4: sprite.direction = 0
    elif sprite.direction == 6: sprite.direction = 2
...
while True:
    ...
    if not game_over:
        ...
        player.last_frame = player.direction * player.columns + player.columns - 1
        ...
        for z in zombie_group:
            ...
            z.last_frame = z.direction * z.columns + z.columns - 1
            if z.frame < z.first_frame:
                z.frame = z.first_frame

            z.velocity = calc_velocity(z.direction)

            # move the zombie and keep it on the screen
            z.X += z.velocity.x
            z.Y += z.velocity.y
        ...
        # check player collision with health
